...
%%
disp('output mat files')
Coord_x=NoFlow_coords.x;
Coord_y=NoFlow_coords.y;
Melt=nanmean(Norm.melt(:,:,:),3).*Base.mask_totten_nan;
Zice=NoFlow_geom.zice;
PercentDiff=(nanmean(Norm.melt,3).*Norm.mask_totten_nan./nanmean(NoFlow.melt(:,:,:),3).*Base.mask_totten_nan-1)*100;

save('../data/proc/matfiles_for_fig_2.mat','Coord_x','Coord_y','Melt','Zice','PercentDiff');
