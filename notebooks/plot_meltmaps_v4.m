...
figure('pos',[600,1000,2000,900])
subaxis(2,4,[1:4],'ML',0.05,'MR',0.08,'MT',0.03,'SV',0.09)
plot(Base.ot,Base.my,'linewidth',2); hold on,
...
legend('Spinup - no flow','no flow','norm','low','high','location','southeast'),legend('boxoff'),grid on,set(gca,'layer','top'),ylabel('melt rate (m/yr)'),xlabel('time (yr)')
...
cmocean('thermal')
ntitle(' b','location','northwest'),ntitle([num2str(nanmean(NoFlow.my(1:12))),' m/yr'],'location','northeast')
...
ntitle(' c Low','location','northwest'),ntitle([num2str((nanmean(Low.my(1:12))/nanmean(NoFlow.my(1:12))-1)*100 ),'%'],'location','northeast')
...
caxis([-10 10]),cmocean('balance','pivot',0)
ntitle(' d Normal','location','northwest'),ntitle([num2str((nanmean(Norm.my(1:12))/nanmean(NoFlow.my(1:12))-1)*100 ),'%'],'location','northeast')
...
caxis([-10 10]),cmocean('balance','pivot',0)
ntitle(' e High','location','northwest'),ntitle([num2str((nanmean(High.my(1:12))/nanmean(NoFlow.my(1:12))-1)*100 ),'%'],'location','northeast')
hCb = colorbar; set(hCb,'pos',[0.928 0.095 0.01 0.4]),ylabel(hCb,'% change')
export_fig MeltComparison -png -transparent -m4
